...
Draft audience: Auth0 builder community

## The practical problem

If you have built an agent-enabled product recently, you have probably shipped some version of this flow:

1. User signs in.
2. User connects Google or Microsoft.
3. Agent can now read and write on the user’s behalf.

That flow works when one app process acts as one principal. It gets shaky when the root agent starts delegating work to child agents.

Our hackathon project, Warrant, started from a narrow question:
...
The short answer: app-level OAuth consent is necessary, but not sufficient.

## Where flat consent breaks

OAuth gives your app delegated tokens. It does not model internal delegation chains.
...
You can still enforce some checks in business logic, but without an explicit delegation contract, your system is hard to reason about and hard to audit.
...
The proof moments are not about "AI magic." They are about control boundaries.
...
Token Vault gives you trustworthy external delegation rails.
A warrant layer gives you legible internal delegation governance.

## Implementation takeaways we found useful
