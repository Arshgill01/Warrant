...
describe("demo fixtures", () => {
  ...
  it("covers the required core planner-to-child flow with concrete seeded examples", () => {
    ...
    expect(examples.commsChildWarrant.status).toBe("blocked");
    ...
    expect(examples.commsChildWarrant.latestBlockedAction?.authorization.code).toBe(
      "capability_missing",
    );
  });
  ...
  it("loads graph and timeline views from the same canonical state and resets safely", () => {
    ...
    expect(graphView.nodes.find((node) => node.id === "warrant-comms-child-001")?.status).toBe("blocked");
