...
export const AgentNodeComponent = memo(({ data, selected }: NodeProps<AgentNode>) => {
  ...
  return (
    <div className={`
      relative min-w-[212px] max-w-[228px] rounded-2xl border bg-white p-3.5 shadow-sm transition-all
      ...
      <div className="mt-4 space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {data.capabilities.map((cap) => (
            <span key={cap} className="rounded-md border border-slate-100 bg-slate-50 px-2 py-1 text-[10px] font-medium text-slate-600">
              {cap}
            </span>
          ))}
        </div>
